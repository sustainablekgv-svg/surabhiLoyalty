...
const CheckoutPage = () => {
  ...
  const fetchCustomer = async () => {
    ...
    if (snap.exists()) {
        console.log("≡ƒöÑ FIRESTORE DATA:", snap.data());
  ...
  const aggregateAdjustedSpv = useMemo(() => {
    ...
    // Formula: ASPV = (Total Adjusted Base ├ù Total SPV) / Total Original Base
    ...
    try {
      if (paymentMethod === 'online') {
        if (totalPayableAmount < 1) {
          toast.error("Online payment requires a minimum amount of Γé╣1. Please use Manual / QR Pay for zero amount orders.");
          setLoading(false);
          return;
        }
      ...
      if (paymentMethod === 'cod') {
        ...
        await addDoc(collection(db, 'Activity'), {
          type: 'order_placed',
          remarks: `New COD Order of Γé╣${orderData.totalAmount.toFixed(2)} placed`,
        ...
        if (!(user as any)?.demoStore && codOrderId) {
          // Fire-and-forget ΓÇö SMS failures must not block checkout.
          ...
          handler: async function (response: any) {
            try {
              ...
              if ((verifyRes.data as any).success) {
                ...
                await addDoc(collection(db, 'Activity'), {
                  type: 'order_placed',
                  remarks: `Online Payment Success for Order Γé╣${totalPayableAmount.toFixed(2)}`,
                  amount: totalPayableAmount,
                  customerName: formData.fullName,
                  customerMobile: formData.mobile,
                  storeLocation: currentStore?.storeName || 'Online',
  ...
  return (
    <ShopLayout title="Checkout" onBack={() => {
      ...
      <div className="max-w-6xl mx-auto px-2 sm:px-4 lg:px-8 py-4 md:py-8">
        ...
        {/* ΓöÇΓöÇ MOBILE STEPPER BAR ΓöÇΓöÇ */}
        ...
        {/* ΓöÇΓöÇ DESKTOP LAYOUT (lg+): unchanged 2-column ΓöÇΓöÇ */}
              ...
              <CardContent className="p-0">
                                ...
                                <React.Fragment key={item.productId}>
                                    ...
                                    <tr className="bg-white">
                                        ...
                                        <td className="px-1.5 py-3 text-right text-sm font-medium text-slate-600 whitespace-nowrap">
                                            Γé╣{item.originalLineTotal.toFixed(2)}
                                        </td>
                                        <td className="px-1.5 py-3 text-right text-sm font-medium text-slate-600 whitespace-nowrap">
                                            Γé╣{item.originalTax.toFixed(2)}
                                        ...
                                        <td className="pl-2 pr-4 py-3 text-right text-sm font-black text-slate-900 whitespace-nowrap">
                                            Γé╣{item.originalTotalInclTax.toFixed(2)}
                                        </td>
                                    </tr>
                                    ...
                                    {redeemedCoinsTotal > 0 && (
                                        <tr className="bg-slate-50/50">
                                            ...
                                            <td className="px-1.5 py-2 text-right text-sm font-black text-slate-900 whitespace-nowrap">
                                                Γé╣{(item.adjustedPrice * item.quantity).toFixed(2)}
                                            </td>
                                            <td className="px-1.5 py-2 text-right text-sm font-black text-slate-900 whitespace-nowrap">
                                                Γé╣{item.adjustedTax.toFixed(2)}
                                                <span className="block text-[9px] text-slate-400 font-black">({item.gstPercentage}%)</span>
                                            </td>
                                            <td className="pl-2 pr-4 py-2 text-right text-sm font-black text-slate-900 whitespace-nowrap">
                                                Γé╣{item.adjustedLineTotal.toFixed(2)}
                                            </td>
                                        </tr>
                                    )}
                                </React.Fragment>
                ...
                <div className="p-6 border-t border-slate-100 space-y-3">
                    <div className="flex items-center justify-between p-3 bg-purple-50 rounded-lg">
                        ...
                        <span className="font-bold text-purple-600">Γé╣{totalOriginalBase.toFixed(2)}</span>
                    </div>

                            {/* ≡ƒöÑ Surabhi Coins Section */}
<div
...
>
  <div className="flex justify-between items-center mb-2">
    ...
    <span className="text-xs font-bold text-slate-500">
      Balance: Γé╣{(customerData?.surabhiBalance || 0).toFixed(2)}
    </span>
  </div>
  ...
  {isCointEligible ? (
    <div className="flex justify-between items-center">
      ...
      <span className="font-bold text-green-600">
        -Γé╣{redeemedCoinsTotal.toFixed(2)}
      </span>
    </div>
  ) : (
...
(customerData?.cumTotal || 0)
      ...
      more to unlock.
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-lg">
                        ...
                        <span className="font-bold text-slate-900">Γé╣{totalAdjustedBase.toFixed(2)}</span>
                    </div>

                    <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-lg">
                        <span className="text-sm font-medium text-slate-700">Adjusted Tax value</span>
                        <span className="font-bold text-slate-900">Γé╣{totalAdjustedTax.toFixed(2)}</span>
                    </div>

                    <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-lg">
                        <span className="text-sm font-medium text-emerald-900">Adjusted Items Total(Incl Tax)</span>
                        <span className="font-bold text-emerald-600">Γé╣{itemsTotalAfterCoins.toFixed(2)}</span>
                    </div>
                        ...
                        {Object.values(productsByGroup).map((group) => (
                            <div key={group.brandName} className="grid grid-cols-2 gap-4 items-center border-b border-indigo-100/30 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
                                ...
                                <div className="text-right">
                                    <span className="text-xs font-black text-slate-900">Γé╣{group.shipping.toFixed(2)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                    ...
                    {/* <div className="flex items-center justify-between p-3 bg-slate-50 border border-slate-100 rounded-lg">
                        ...
                        <span className={`font-bold ${(user as any)?.shippingBalance > 0 ? 'text-blue-600' : 'text-red-600'}`}>
                            Γé╣{((user as any)?.shippingBalance || 0).toFixed(2)}
                        </span>
                    </div> */}

                    <div className="flex items-center justify-between p-3 bg-indigo-50 rounded-lg">
                        <span className="text-sm font-medium text-indigo-900">Total Estimated Delivery Charges</span>
                        <span className="font-bold text-indigo-600">Γé╣{shippingCost.toFixed(2)}</span>
                    </div>
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] text-blue-600 font-medium">
                                        Applied from Balance: Γé╣{shippingCreditsUsed.toFixed(2)}
                                    </p>
                                </div>
                                <span className="text-xs font-bold text-blue-700 bg-blue-100 px-2 py-1 rounded-full">
                                    Balance: Γé╣{maxShippingCreditsAvailable.toFixed(2)}
                                </span>
                            </div>
                        </div>
                    )} */}
                    ...
                    {shippingCreditsUsed !== 0 && (
                        <div className={`flex items-center justify-between p-3 rounded-lg ${shippingCreditsUsed > 0 ? 'bg-blue-50' : 'bg-red-50'}`}>
                            ...
                            <span className={`font-bold ${shippingCreditsUsed > 0 ? 'text-blue-600' : 'text-red-600'}`}>
                                {shippingCreditsUsed > 0 ? '-' : '+'}Γé╣{Math.abs(shippingCreditsUsed).toFixed(2)}
                            </span>
                        </div>
                    )}
                         ...
                         <div className="flex justify-between items-end mb-6">
                             <div>
                                 ...
                                 <h2 className="text-5xl font-black text-slate-900 leading-none mt-2">
                                     Γé╣{totalPayableAmount.toFixed(2)}
                                 </h2>
                    ...
                    <div className="pt-8 border-t border-slate-200">
                        ...
                        <div className="space-y-3">
                             ...
                             <div className="flex justify-between text-sm font-black">
                                 ...
                                 <span className="text-slate-900">Γé╣{totalShippingCreditsEarned.toFixed(2)}</span>
                             </div>
                             <div className="flex justify-between text-sm font-black">
                                 <span className="text-slate-600">Seva Pool ({currentStore?.sevaCommission || 0}%)</span>
                                 <span className="text-slate-900">Γé╣{sevaPoolEarned.toFixed(2)}</span>
                             </div>
            ...
            <Card className="border shadow-sm bg-white rounded-xl overflow-hidden">
              ...
              <CardContent className="p-0">
                      ...
                      {adjustedCart.map(item => (
                        <React.Fragment key={item.productId}>
                          <tr className="bg-white">
                            ...
                            <td className="pl-1 pr-3 py-2 text-right text-xs font-black text-slate-900 whitespace-nowrap">Γé╣{item.originalTotalInclTax.toFixed(2)}</td>
                          </tr>
                          {redeemedCoinsTotal > 0 && (
                            <tr className="bg-slate-50/50">
                              ...
                              <td className="pl-1 pr-3 py-1 text-right text-xs font-black text-slate-900 whitespace-nowrap">Γé╣{item.adjustedLineTotal.toFixed(2)}</td>
                            </tr>
                          )}
                        </React.Fragment>
                      ))}
                ...
                <div className="p-4 border-t border-slate-100 space-y-2">
                  <div className="flex justify-between"><span className="text-xs text-slate-600">Items (Excl Tax)</span><span className="font-bold text-sm">Γé╣{totalOriginalBase.toFixed(2)}</span></div>
                  {isCointEligible && redeemedCoinsTotal > 0 && (<div className="flex justify-between text-green-700"><span className="text-xs font-medium">Coins Applied</span><span className="font-bold text-sm">-Γé╣{redeemedCoinsTotal.toFixed(2)}</span></div>)}
                  {!isCointEligible && (<div className="text-[10px] text-red-600 bg-red-50 p-2 rounded-lg">Spend Γé╣{Math.max(0,(customerData?.cummulativeTarget||0)-(customerData?.cumTotal||0)).toFixed(2)} more to unlock coins.</div>)}
                  ...
                  <div className="flex justify-between pt-1 border-t"><span className="text-xs text-slate-600">Est. Delivery</span><span className="font-bold text-sm text-indigo-600">Γé╣{shippingCost.toFixed(2)}</span></div>
                  ...
                  <div className="flex justify-between items-end pt-2 border-t-2 border-slate-200">
                    <div><p className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Total Payable</p><h2 className="text-3xl font-black text-slate-900 leading-none mt-1">Γé╣{totalPayableAmount.toFixed(2)}</h2><p className="text-[9px] text-slate-400 font-bold mt-1">Incl. GST &amp; Delivery</p></div>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        {mobileStep === 3 && (
          <div className="lg:hidden space-y-4">
            ...
            <div className="bg-white rounded-xl border p-4 space-y-2">
              <div className="flex justify-between text-sm"><span className="text-slate-600">Items Total</span><span className="font-bold">Γé╣{itemsTotalAfterCoins.toFixed(2)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-slate-600">Delivery</span><span className="font-bold text-indigo-600">Γé╣{shippingCost.toFixed(2)}</span></div>
              <div className="flex justify-between items-end pt-2 border-t"><span className="text-xs font-black uppercase text-slate-400 tracking-wider">Total Payable</span><span className="text-2xl font-black text-slate-900">Γé╣{totalPayableAmount.toFixed(2)}</span></div>
            </div>
