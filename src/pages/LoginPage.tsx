...
const LoginPage = () => {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          if (formData.role === 'customer') {
            ...
            // Check if any document has the matching role
            const hasMatchingRole = querySnapshot.docs.some(doc => doc.data().role === formData.role);
            if (hasMatchingRole) {
              setMobileStatus('exists');
            } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mobileStatus === 'not_found') {
      toast.error('This mobile number is not registered for this role. Please verify or sign up.');
      return;
    }
                    ...
                    <>
                      ...
                      {!isCheckingMobile && mobileStatus === 'not_found' && (
                        <p className="text-xs text-red-500 font-medium flex items-center gap-1">
                          ⚠️ Mobile number is not registered for this role.
                        </p>
                      )}
