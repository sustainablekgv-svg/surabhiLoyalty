...
export const ProductCard: React.FC<ProductCardProps> = ({ product, variant = 'default' }) => {
  ...
  return (
    <Card className="group overflow-hidden border-0 bg-transparent shadow-none hover:shadow-lg transition-all duration-300 rounded-xl bg-white flex flex-col h-full">
      ...
      <div className="relative aspect-square overflow-hidden bg-white cursor-pointer flex items-center justify-center" onClick={() => { window.scrollTo(0, 0); navigate(`/shop/product/${product.id}`, { state: { from: location.pathname + location.search } }); }}>
      ...
      <CardContent className="p-4 flex-1 flex flex-col">
        ...
        <h3 
          ...
          onClick={() => { window.scrollTo(0, 0); navigate(`/shop/product/${product.id}`, { state: { from: location.pathname + location.search } }); }}
          title={product.name}
        >
          {product.name}
        </h3>
